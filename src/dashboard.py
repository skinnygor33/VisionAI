...
DEFAULT_PATH = os.path.join(BASE_DIR, "..", "data","test", "synthetic_test_large.csv")
                ...
                if response.status_code != 200:
                    ...
                    try:
                        ...
                        if "results" not in data:
                            ...
                            st.subheader("Predicted Risk Scores Overview")
                            st.bar_chart(df_out['predicted_risk'])

                            # --- Scatter plots ---
                            if "Amount" in df_out.columns:
                                st.subheader("Risk vs Amount")
                                st.scatter_chart(df_out[["Amount", "predicted_risk"]])
